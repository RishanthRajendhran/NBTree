...
from helper.imports.functionImports import replace_null_values_with_mean, replace_null_values_with_mode, standardize, mapOrdinalCategoricalValuesToNumbers, convert_given_cols_to_one_hot, get_correlation_matrix, select_features
...
def preprocessTrainData(X, weights, Y, colVals, handleMulticolinearity, performFeatureSelection=False):
    ...
    X, weights = replace_null_values_with_mean(X, weights, dataConfig.continuous_feature_indices)
    # X, weights, colMeansCont, colSTDsCont = standardize(X, weights, dataConfig.continuous_feature_indices,[],[])

    X, weights = replace_null_values_with_mean(X, weights, dataConfig.discrete_feature_indices)
    # X, weights, colMeansDisc, colSTDsDisc = standardize(X, weights, dataConfig.discrete_feature_indices,[],[])

    #Categorical attributes

    X, weights = replace_null_values_with_mode(X, weights, "str", dataConfig.ordinal_feature_indices)
    X, weights, colModesOrdinal = mapOrdinalCategoricalValuesToNumbers(X, weights, dataConfig.ordinal_feature_indices)

    X, weights = replace_null_values_with_mode(X, weights, "str", dataConfig.nominal_feature_indices)
